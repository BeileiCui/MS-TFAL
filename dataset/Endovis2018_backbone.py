...
class endovis2018(Dataset):
    ...
    def __init__(self, split, t=1, arch='swinPlus', rate=1, global_n=0, data_ver=0, h=512, w=640):
        ...
        if self.test_part:
            ...
            # test_images = [[i,s] for s in range(1, 2) for i in range(1,45)] #250
            test_images_2 = [[i,s] for s in range(2,5) for i in range(1,249,4)] #249
            # test_images_2 = [[i,s] for s in range(2,5) for i in range(1,45)] #249
            self.images = test_images +test_images_2
            print(f'Loaded {len(self.images)} noisy frames. For {self.mode}')
...
#             label_1 = tsf_1['mask']
        ...
        label_1 = label_1[::self.rate, ::self.rate]
    ...
    def _load_data(self, ins, frame, t=1, global_n=0):
        ...
        if self.test or self.test_part:
            imgs_t = [np.array(i) for i in imgs_t]
            imgs_t_1 = [np.array(i) for i in imgs_t_1]
            ...
            # imgs_save = imgs
            # masks_save = np.array(masks) * 15
            # if self.test:
            #     for i in range(len(imgs)):
            #         savepath = os.path.join(DATA_ROOT, 'input image check/test_{}_image.png')
            #         cv2.imwrite(savepath.format(i), imgs_save[i])
            #     cv2.imwrite(os.path.join(DATA_ROOT, 'input image check/test_mask.png'), masks_save)
            # elif self.test_part:
            #     for i in range(len(imgs)):
            #         savepath = os.path.join(DATA_ROOT, 'input image check/test_part_{}_image.png')
            #         cv2.imwrite(savepath.format(i), imgs_save[i])
            #     cv2.imwrite(os.path.join(DATA_ROOT, 'input image check/test_part_mask.png'), masks_save)
            #print('!!',np.unique(masks))
            #print('Full resolution for test data!')


        elif self.mode == 'train' or 'train_clean':
            masks = Image.open(r_lb.format(ins, frame))
            masks_1 = Image.open(r_lb.format(ins, frame_1))
            ...
            # imgs_save = [np.array(i) for i in imgs]
            # masks_save = np.array(masks) * 15
            # for i in range(len(imgs)):
            #     savepath = os.path.join(DATA_ROOT, 'input image check/train_{}_image.png')
            #     cv2.imwrite(savepath.format(i), imgs_save[i])
            # cv2.imwrite(os.path.join(DATA_ROOT, 'input image check/train_mask.png'), masks_save)
            # print(imgs[0].shape, masks.shape)
            # print(1 + '1')
            # imgs_t, imgs_t_1, masks, masks_1 = self._random_scale(imgs_t, imgs_t_1, masks, masks_1)
        for i in range(len(imgs_t)):
            imgs_t[i] = np.array(imgs_t[i])
            imgs_t_1[i] = np.array(imgs_t_1[i])
        masks = np.array(masks)
        masks_1 = np.array(masks_1)
        return imgs_t, imgs_t_1, masks.astype('uint8'), masks_1.astype('uint8')
