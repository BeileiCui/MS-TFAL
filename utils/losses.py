...
class myOhemCELoss2D(nn.CrossEntropyLoss):
    """2D Cross Entropy Loss with Auxilary Loss"""
    def __init__(self,n_min,thresh=0.7,ignore_index=-1,):

        super(myOhemCELoss2D, self).__init__(None, None, ignore_index, reduction='none')

        self.thresh = -math.log(thresh)
        self.n_min = n_min #最少要计算多少个pixel的loss
        self.ignore_index = ignore_index

    def forward(self, pred, target,remove_map):
        return self.OhemCELoss(pred, target,remove_map)

    def OhemCELoss(self, logits, labels,remove_map=None):
        loss = super(myOhemCELoss2D, self).forward(logits, labels).view(-1)

        if remove_map is not None:
            remove_map = remove_map.view(-1)

            assert loss.shape[0] == remove_map.shape[0]

            loss = loss[remove_map!=1]
